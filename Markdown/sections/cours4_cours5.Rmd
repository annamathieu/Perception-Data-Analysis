...
library(FactoMineR)
library(here)
donnees <- read_xls(here("Markdown/dataset","Descriptive.xls"))
summary(donnees)
...
Tri <- read.table(here("Markdown/dataset","trih.txt"), header = TRUE, stringsAsFactors = TRUE)
...
Pour réaliser l'AFMH, on doit définir la hiérarchie au travers des variable. Ici, dès qu'il y a un "H1" dans le nom d'une des variables, cela signifie qu'on passe à un nouvel enfant (avec sa première hiérarchie). On crée donc un vecteur de taille "nombre d'enfant" avec pour chaque valeur, le nombre de partitions réalisées par un enfant.

Puisque le jeu de donnée est trop grand, on va étudier seulement une partie des enfants.
...
```{r}
load(here("Markdown/dataset","resultat_AFMH.RData"))

coord <- res.AFMH$group$coord
coord <- coord[[1]]
```


Affichons les résultats de l'AFMH

```{r}
plot.HMFA(res.AFMH, choix = "ind", invisible = "quali", label = "none")
```
...
coord$Enfant <- as.factor(rep(c(1:170), times = as.integer(res)))

fig1 <- coord %>% group_by(Enfant) %>% 
  ggplot() +
  aes(x = Dim.1, y = Dim.2) +
  geom_point(aes(col = Enfant), alpha = 05.) + geom_line(aes(group = Enfant, col = Enfant)) +
